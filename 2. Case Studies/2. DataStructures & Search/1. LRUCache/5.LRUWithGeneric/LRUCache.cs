...
public class LRUCache<TKey, TValue> : IDisposable where TKey : notnull
{
    ...
    public TValue Get(TKey key)
    {
        lock (_cacheLock)
        {
            if (_map.TryGetValue(key, out Node<TKey, TValue>? node) == false)
    ...
    public void Put(TKey key, TValue value, DateTimeOffset expiresAt)
    {
        ...
        lock (_cacheLock)
        {
            ...
            else
            {
                ...
                // Insert the new node
                Node<TKey, TValue> newNode = new Node<TKey, TValue>(key, value, expiresAt);
        ...
        while (true)
        {
            try
            {
                ...
                lock (_cacheLock)
                {
                    ...
                    List<Node<TKey, TValue>> expiredKeys = new List<Node<TKey, TValue>>();
                    ...
                    foreach (Node<TKey, TValue> node in expiredKeys)
                    {
    ...
    {
        Node<TKey, TValue> lastNode = _list.Last;
